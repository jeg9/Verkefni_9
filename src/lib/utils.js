...
export function element(name, attributes = null, events = null, ...children) {
    const elmnt = document.createElement(name);
    ...
    for (const child of children) {
        ...
        if (attributes) {
            for (const attrib in attributes) {
                elmnt.setAttribute(attrib, attributes[attrib]);
            }
        }

        if (events) {
            for (const event in events) {
                elmnt.addEventListener(event, events[event]);
            }
        }

        if (typeof child === 'string') {
            elmnt.appendChild(document.createTextNode(child));
        } else {
